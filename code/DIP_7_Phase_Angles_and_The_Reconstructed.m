...
F_rect = fft2(rect);
F_new = fft2(img_new);
phase_rect = exp(1i*angle(F_rect));
phase_new = exp(1i*angle(F_new));
spectrum_rect = abs(F_rect);
spectrum_new = abs(F_new);

%% reconstruction by my phase + rectangle spectrum
reconstruction_1 = phase_new .* spectrum_rect; % 오일러 공식에 의해 exp 형태로 바꾼 후 페이즈를 더하기 때문에 각각 곱연산
...
reconstruction_2 = phase_rect .* spectrum_new;
